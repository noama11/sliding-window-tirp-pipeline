...
> | The `all_patients` variant (`pipeline_all_patients.py` — no YES/NO split), needed to give the prediction model a population-wide matrix | **done** — `--cohort all` | `HANDBOOK.md` §5 |
>
> `--cohort all` keeps the Y-derived window plan rather than the SQL variant's
> 2000-onward K-only one, so its matrices index the same windows the YES/NO run
> labelled. MVS stays at the config value (the SQL variant hard-coded 0.3);
> `--mvs` overrides it.
>
