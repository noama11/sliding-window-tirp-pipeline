"""
TIRP Pipeline - All Patients (Sliding Window, No YES/NO Split)
Run: python pipeline_all_patients.py
Config: config_all_patients.json
"""
...
def generate_windows():
    """Generate all (k_start, k_end) windows."""
...
def get_all_patients(k_start, k_end):
    """Get all patients who have any record in the K window."""
    query = f"""
        SELECT DISTINCT PatientID
        FROM [{INPUT_DATABASE}].[dbo].[InputPatientsData]
        WHERE StartTime >= '{k_start}-01-01' AND StartTime < '{k_end}-01-01'
    """
    conn = get_db_connection()
...
def run_karmalego(results_path):
    ...
    # Check output files recursively
...
def process_window(k_start, k_end, window_num, total):
    """Process a single window - all patients."""
    log(f"")
    log(f"========== WINDOW {window_num}/{total}: K=[{k_start}-{k_end}] ==========")
    ...
    # Get all patients
    log("--- STEP 1: FIND ALL PATIENTS ---")
    patients = get_all_patients(k_start, k_end)
    ...
    results_path = f"{RESULTS_BASE_PATH}\\{k_start}-{k_end}_ALL_patterns"
...
def run_pipeline():
    ...
    log(f"")
    log(f"============================================")
    log(f"TIRP PIPELINE STARTING - ALL PATIENTS")
    log(f"Config: K={K}, STEP={STEP}, MVS={MVS}")
    ...
    for i, (k_start, k_end) in enumerate(windows, 1):
        process_window(k_start, k_end, i, len(windows))
