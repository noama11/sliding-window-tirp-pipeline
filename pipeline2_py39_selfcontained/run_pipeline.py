...
There is a second cohort mode, `--cohort all`, which drops the split entirely:

    ALL cohort = every patient with any record in the K-window
        -> Mediator(K-window) -> KarmaLego -> results/<k_start>-<k_end>_<P>ALL_patterns

It is the equivalent of the original pipeline_all_patients.py, and exists
because the prediction model needs one pattern-support row per patient over the
whole population, not two cohort-local matrices. The window plan is the SAME in
both modes -- ALL still stops where the Y-window would run past END_DATE -- so
every ALL row lines up with a K-window that the YES/NO run also labelled.
...
research room has and cannot be upgraded from.
...
# "yes-no" = the two-cohort run the outcome models were built on. "all" = one
# cohort of every patient in the K-window, no outcome involved -- what the
# prediction model needs a full pattern-support matrix from.
COHORT_MODES = ("yes-no", "all")
...
class Settings:
    ...
    def __init__(self, cfg, label=None, data_dir=None, results_dir=None,
        ...
        self.SEED = c["SEED"]
        self.COHORT_MODE = (cohort_mode or c.get("MODE", "yes-no")).strip().lower()
        if self.COHORT_MODE not in COHORT_MODES:
            die(f"unknown cohort mode {self.COHORT_MODE!r}. "
                f"Use one of: {', '.join(COHORT_MODES)}")
        ...
        with open(S.RAW_EVENTS, encoding="utf-8-sig", newline="") as f:
            ...
            for row in r:
                ...
                if not pid:
                    continue
                all_p.add(pid)
                # Every timestamp format _parse_dt accepts starts with the year,
                # so the first four characters give it. Slicing beats parsing
                # here: this runs on every row of a multi-million-row export,
                # and the ALL cohort only ever asks which years a patient has.
                y = row[2].strip()[:4]
                if y.isdigit():
                    years.setdefault(pid, set()).add(int(y))
                if row[1].strip().lower() == target:
                    dt = _parse_dt(row[2])
                    if dt is not None:
                        events.append((pid, dt))
        _RAW_CACHE["idx"] = (all_p, events, years)
        log(f"  indexed {len(all_p)} patients, {len(events)} '{S.EVENT_CONCEPT}' "
            f"events in {time.time() - t0:.0f}s")
        if not events and S.COHORT_MODE != "all":
            # Irrelevant in ALL mode: no cohort there depends on EVENT_CONCEPT.
...
def get_positive_patients(S, y_start, y_end):
    ...
    _all, events, _years = load_raw_index(S)
    return sorted({pid for pid, dt in events if lo <= dt < hi},
                  key=lambda x: (len(x), x))


def get_all_patients(S, k_start, k_end):
    """ALL cohort: every patient with at least one record whose StartTime is in
    the K-window [k_start-01-01, k_end-01-01) -- the rule
    pipeline_all_patients.py applied to InputPatientsData.

    Note this is the LOCAL year boundary, not the local->UTC clamped window the
    Mediator abstracts over (see mediator_window). Cohort membership is a
    selection rule rather than an abstraction, and the SQL pipeline selected on
    the plain year, so the two stay comparable. A patient whose only rows fall
    in the two hours the clamp shaves off the window still joins the cohort and
    simply contributes no abstractions."""
    _all, _events, years = load_raw_index(S)
    span = range(k_start, k_end)
    return sorted((pid for pid, ys in years.items() if not ys.isdisjoint(span)),
                  key=lambda x: (len(x), x))


def get_negative_patients(S, count):
    """NO cohort: `count` patients who NEVER had EVENT_CONCEPT, seeded sample."""
    all_p, events, _years = load_raw_index(S)
...
def process_window(S, k_start, k_end, y_start, y_end, idx, total):
    if S.COHORT_MODE == "all":
        return process_window_all(S, k_start, k_end, idx, total)
...
def process_window_all(S, k_start, k_end, idx, total):
    """One cohort, no outcome: everyone with data in the K-window.

    The Y-window is not consulted here -- it only decided which K-windows exist
    (generate_windows), so that these matrices index the same windows the YES/NO
    run labelled."""
    log("")
    log(f"########## WINDOW {idx}/{total}: K=[{k_start}-{k_end}]  (ALL patients) ##########")

    patients = get_all_patients(S, k_start, k_end)
    log(f"ALL (any record in K): {len(patients)} patients")
    if not patients:
        log("No patients with data in this K-window -- skipping window.")
        return

    prepare_window_data(S, [("ALL", patients)])
    log("--- ALL cohort ---")
    run_leg(S, "ALL", patients, k_start, k_end)
    log(f"Window {k_start}-{k_end} complete.")
...
def main():
    ...
    p.add_argument("--cohort", choices=COHORT_MODES,
                   help="'yes-no' (default): the YES and NO cohorts of each window. "
                        "'all': one cohort per window holding every patient with data "
                        "in the K-window, no outcome split -- results land in "
                        "<k_start>-<k_end>_<PREFIX>ALL_patterns.")
    ...
    p.add_argument("--mvs", type=float,
                   help="Override karmalego.MVS (minimum vertical support, a FRACTION "
                        "of the cohort). Worth setting for --cohort all: the same "
                        "fraction of every patient is a far higher absolute bar than it "
                        "is on a YES cohort of a few hundred.")
    args = p.parse_args()

    S = Settings(cfg, label=args.label, data_dir=args.data_dir,
                 results_dir=args.results_dir, cohort_mode=args.cohort)
    if args.max_level:
        S.MAX_LEGO_LEVEL = args.max_level
    if args.mvs is not None:
        S.MVS = args.mvs
    ...
    if args.list_windows:
        if S.COHORT_MODE == "all":
            print(f"label: {S.label}  (cohort: all patients -- event concept unused)")
        else:
        ...
        for ks, ke, ys, ye in windows:
            if S.COHORT_MODE == "all":
                # Y is still printed: it is why the list ends where it does.
                print(f"  K=[{ks}-{ke}]  (Y=[{ys}-{ye}])  -> "
                      f"{ks}-{ke}_{S.RESULTS_PREFIX}ALL_patterns")
            else:
                print(f"  K=[{ks}-{ke}]  Y=[{ys}-{ye}]  -> "
    ...
    log(f"label  : {S.label}  ->  event concept "
        f"{'(unused in ALL mode)' if S.COHORT_MODE == 'all' else repr(S.EVENT_CONCEPT)}, "
        f"results prefix '{S.RESULTS_PREFIX or '(none)'}'")
    log(f"window : K={S.K}y  Y={S.Y}y  STEP={S.STEP}y  "
        f"START={S.START_YEAR}  END={S.END_DATE}")
    if S.COHORT_MODE == "all":
        log("cohort : ALL patients with data in the K-window (no YES/NO split)")
    else:
    ...
    if args.window is not None:
        ...
        log(f"Running {len(windows)} window(s), "
            + ("one ALL cohort each." if S.COHORT_MODE == "all" else "YES + NO each."))
