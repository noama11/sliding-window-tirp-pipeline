...
class Engine:
    ...
    def load_raw_csv(self, path):
        ...
        point_event_concepts = {
            "Bleeding",
            "Nontraumatic_Intracerebral_Hemorrhage",
            "Nontraumatic_Subarachnoid_Hemorrhage",
            "Other_Nontraumatic_Hemorrhage",
        }

        with open(path, newline="", encoding="utf-8-sig") as fh:
            reader = csv.DictReader(fh)

            # Normalize CSV headers because the source export may contain
            # trailing spaces, for example "Value ".
            ...
            for d in reader:
                ...
                start_time = str(d["StartTime"]).strip()
                end_time = str(d["EndTime"]).strip()
                value = str(d["Value"]).strip()

                # These raw nominal events are exported without EndTime.
                # Represent them as point events without inventing a duration.
                if (
                    not end_time
                    and start_time
                    and value.lower() == "true"
                    and name in point_event_concepts
                ):
                    end_time = start_time
